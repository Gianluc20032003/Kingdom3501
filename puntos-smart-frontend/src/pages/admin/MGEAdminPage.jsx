  ...
  const handleMGEConfig = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showAlert(
          response.message || "Error al guardar configuración",
          "error"
        );
      }
    } catch (error) {
      ...
      <form onSubmit={handleMGEConfig} className="space-y-6">
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setMgeConfig((prev) => ({ ...prev, tipo_tropa: e.target.value }))
            }
                ...
                >
                  ...
                  <td className="px-4 py-4 text-center">
                    <span className="inline-flex items-center px-2 py-1 bg-purple-100 text-purple-800 rounded-full text-xs font-semibold">
                      <span className="mr-1">
                        {getTipoTropaIcon(postulacion.tipo_tropa)}
                      </span>
                  ...
                  <td className="px-4 py-4 text-center">
                    <span className="text-sm font-medium">
                      {postulacion.comandante_principal}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-center">
                  ...
                  <td className="px-4 py-4 text-center">
                    {postulacion.foto_inscripciones_url ? (
                      <img
                        src={getImageUrl(postulacion.foto_inscripciones_url)}
                        alt="Inscripciones"
                        className="w-12 h-12 object-cover rounded mx-auto cursor-pointer hover:opacity-80 transition-opacity"
                        onClick={() =>
                          openImageModal(
                            getImageUrl(postulacion.foto_inscripciones_url)
                          )
                        }
                      />
                    ) : (
                  ...
                  <td className="px-4 py-4 text-center">
                    {postulacion.foto_comandantes_url ? (
                      <img
                        src={getImageUrl(postulacion.foto_comandantes_url)}
                        alt="Comandantes"
                        className="w-12 h-12 object-cover rounded mx-auto cursor-pointer hover:opacity-80 transition-opacity"
                        onClick={() =>
                          openImageModal(
                            getImageUrl(postulacion.foto_comandantes_url)
                          )
                        }
                      />
                    ) : (
                  ...
                  <td className="px-4 py-4 text-center">
                    {postulacion.foto_cabezas_url ? (
                      <img
                        src={getImageUrl(postulacion.foto_cabezas_url)}
                        alt="Cabezas"
                        className="w-12 h-12 object-cover rounded mx-auto cursor-pointer hover:opacity-80 transition-opacity"
                        onClick={() =>
                          openImageModal(
                            getImageUrl(postulacion.foto_cabezas_url)
                          )
                        }
                      />
                    ) : (
